...
public class Member extends BaseEntity {
	...
	private void validateName(String name) {
		...
		if (!onlyContainsCharacters) {
			throw new InvalidDataException(
					MessageFormat.format("입력된 이름 : {}. 이름에는 문자만 입력 가능합니다.", name));
		}
		if (name.length() > 10) {
			throw new InvalidDataException(
					MessageFormat.format("입력된 이름 : {}자. 이름은 최대 {}자 입력 가능합니다.", name.length(), NAME_MAX_LENGTH));
		}
	}
	...
	void validateAge(int age) {
		if (!(0 < age && age <= MAX_AGE)) {
			throw new InvalidDataException(
					MessageFormat.format("입력된 나이 : {}. 나이는 0 이상 {} 이하의 숫자로 입력해주세요.", age, MAX_AGE));
		}
	}
	...
	private void validateHobby(String hobby) {
		if (hobby.length() > 100) {
			throw new InvalidDataException(
					MessageFormat.format("입력된 취미 : {}자. 취미는 최대 {}자 입력 가능합니다.", hobby.length(), HOBBY_MAX_LENGTH));
		}
	}
